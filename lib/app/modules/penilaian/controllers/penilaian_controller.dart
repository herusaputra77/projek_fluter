...
class PenilaianController extends GetxController {
  ...
  Future<List<dynamic>> Datakuis() async {
    var response = await http.get(
      Uri.parse("http://10.0.2.2:8000/api/tampil/kuis"),
      headers: {
  ...
  void saveDataToDatabase(int score, String username) async {
    ...
    var response = await http.post(
      Uri.parse('http://10.0.2.2:8000/api/hasil/kuis'),
      headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer ${box.read('token')}',
      },
    ...
    if (response.statusCode == 200) {
      ...
      // var requestData = {
      //   'score': score.toString(),
      //   'username': '${_loginController.userControl.text}',
      // };

      // http
      //     .post(Uri.parse('http://10.0.2.2:8000/api/hasil/kuis'),
      //         headers: {
      //           'Accept': 'application/json',
      //         },
      //         body: requestData)
